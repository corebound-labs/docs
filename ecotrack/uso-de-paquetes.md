...
| [Commons.Notifications](../packages/commons-notifications.md) | `AddNotifications().AddInAppChannel<EcoTrackDbContext>().AddPreferences<EcoTrackDbContext>().AddPurgeJob(30)` en `Program.cs`; `ApplyNotificationsModel()` en `EcoTrackDbContext` (migración `AddNotifications`). Los textos y tipos de aviso viven en `EcoTrack.Application` (ver [Notificaciones](notifications.md)). |
