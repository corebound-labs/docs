...
| [Commons.Notifications](../packages/commons-notifications.md) | `AddNotifications().AddInAppChannel<EcoTrackDbContext>()`. Tres disparadores: `import.finished` al terminar un `ImportJob`, `SettlementNotificationService` (liquidaciones entre cotitulares) y `SharingNotificationService` (compartir transacción/billetera). Nunca dispara en reparto residual automático ni en transacciones generadas por plantillas programadas. Limpieza de avisos wired en `DeactivateUserAccountsHandler`. |
| [UiMetadata.Notifications](../packages/uimetadata-notifications.md) | `AddUiMetadataNotifications()` + `MapNotificationsHub()`. Campana en el layout de todas las pantallas; evento `uinotification` en `Transaction/Index.cshtml` refresca la grilla en silencio cuando llega `import.finished`. |
